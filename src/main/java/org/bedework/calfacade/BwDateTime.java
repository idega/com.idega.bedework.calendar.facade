...
public class BwDateTime extends DumpEntity<BwDateTime>
  ...
  public static BwDateTime makeBwDateTime(final boolean dateType,
                                          final String date,
                                          final String tzid,
                                          final TimeZone tz) throws CalFacadeException {
    try {
      if (dateType) {
      ...
      if (!bwd.getFloating()) {
        bwd.setDate(Timezones.getTimezones().getUtc(date, tzid, tz));
      }

      return bwd;
    } catch (Timezones.TimezonesException tze) {
      if (tze.getMessage().equals(Timezones.TimezonesException.unknownTimezone)) {
  ...
  public static BwDateTime makeBwDateTime(final boolean dateType,
    ...
    return makeBwDateTime(dateType, date, tzid, null);
  }
  ...
  public static BwDateTime makeBwDateTime(final Date val) throws CalFacadeException {
    ...
    if (val instanceof DateTime) {
      dateType = false;
      tz = ((DateTime)val).getTimeZone();
      if (tz != null) {
        tzid = tz.getID();
      }
    }

    return makeBwDateTime(dateType, val.toString(), tzid, tz);
  }
